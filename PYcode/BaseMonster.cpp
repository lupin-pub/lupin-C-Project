...
void BaseMonster::hpSchedule(float dt)//1.0f
{
...
void BaseMonster::checkNearestMonster()
{
	...
	if (this->getMonsterColor() == 1)
	{
		...
		monsterDistance = currMinDistant;
...
void BaseMonster::shoot(float dt)//每隔多久shoot一次 0.5
{
	...
	{
		cocos2d::Action* moveByAction = MoveBy::create(0.5f, Point(SoldierMoveBy.x,SoldierMoveBy.y));//行吗
