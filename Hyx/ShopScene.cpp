...
bool ShopScene::onTouchAttactBegan(Touch * touch, Event * event) {
	...
	if (rect.containsPoint(locationInNode)) {
	    ...
	    //清楚上一次装备所改的属性
	    //改变这次属性
		if (target->getMoney() <= baseSprite::getMoney()) {
			//购买成功
			if (shopIconsAttact.empty()) {
				//第一次购买
				//BaseMonster::changeArmor(target -> );
				BaseMonster::changeCurrHp(target->getHP());
				BaseMonster::changeForce(target->getAttact());
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
				shopIconsAttact.push_back(target);
			}
			else {
				//非第一次购买
				ShopAttactIcon* lastIcon = shopIconsAttact.back();
				BaseMonster::changeCurrHp(-(lastIcon->getHP()));
				BaseMonster::changeForce(-(lastIcon->getAttact()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				shopIconsAttact.pop_back(lastIcon);
				BaseMonster::changeCurrHp(target->getHP());
				BaseMonster::changeForce(target->getAttact());
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
...
bool ShopScene::onTouchDefenseBegan(Touch * touch, Event * event) {
	...
	if (rect.containsPoint(locationInNode)) {
		...
		//清楚上一次装备所改的属性
		//改变这次属性
		if (target->getMoney() <= BaseMonster::getMoney()) {
			//购买成功
			if (shopIconsDefense.empty()) {
				...
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
				shopIconsDefense.push_back(target);
			}
			else {
				//非第一次购买
				...
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				shopIconsDefense.pop_back(lastIcon);
				BaseMonster::changeCurrHp(target->getHP());
				BaseMonster::changeForce(target->getAttact());
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
	...
	if (rect.containsPoint(locationInNode)) {
		...
		if (target->getMoney() <= BaseMonster::getMoney()) {
			...
			if (shopIconsSpell.empty()) {
				...
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
				shopIconsSpell.push_back(target);
			}
			else {
				//非第一次购买
				ShopAttactIcon* lastIcon = shopIconsSpell.back();
				BaseMonster::changeCurrHp(-(lastIcon->getHP()));
				BaseMonster::changeForce(-(lastIcon->getAttact()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				shopIconsSpell.pop_back(lastIcon);
				BaseMonster::changeCurrHp(target->getHP());
				BaseMonster::changeForce(target->getAttact());
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
	...
	if (rect.containsPoint(locationInNode)) {
		...
		if (target->getMoney() <= BaseMonster::getMoney()) {
			...
			if (shopIconsMove.empty()) {
				...
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
				shopIconsMove.push_back(target);
			}
			else {
				//非第一次购买
				...
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				shopIconsMove.pop_back(lastIcon);
				BaseMonster::changeCurrHp(target->getHP());
				BaseMonster::changeForce(target->getAttact());
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
	...
	if (rect.containsPoint(locationInNode)) {
		...
		if (target->getMoney() <= BaseMonster::getMoney()) {
			...
			if (shopIconsPlayingWild.empty()) {
				...
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
				shopIconsPlayingWild.push_back(target);
			}
			else {
				//非第一次购买
				...
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				BaseMonster::changeRunSpeed(-(lastIcon->getSpeed()));
				shopIconsPlayingWild.pop_back(lastIcon);
				BaseMonster::changeCurrHp(target->getHP());
				BaseMonster::changeForce(target->getAttact());
				BaseMonster::changeRunSpeed(target->getSpeed());
				BaseMonster::changeRunSpeed(target->getSpeed());
...
void ShopScene::initDefensePicture()
{
	...
	auto buttonListener = EventListenerTouchOneByOne::create();
	buttonListener->setSwallowTouches(true);
	buttonListener->onTouchBegan = CC_CALLBACK_2(ShopScene::onTouchDenfenseBegan, this);
	EventDispatcher* _eventDispatcher = Director::getInstance()->getEventDispatcher();
