...
//添加更改属性的函数
//通过控制temp 的正负更改
void BaseMonster::changeCurrHp(int temp) {
	currHp += temp;
}
void BaseMonster::changeMaxHp(int temp) {
	maxHp += temp;
}
void BaseMonster::changeRunSpeed(int temp) {
	runSpeed += temp;
}
void BaseMonster::changeScope(int temp) {
	scope += temp;
}
void BaseMonster::changeForce(int temp) {
	force += temp;
}
void BaseMonster::changeArmor(int temp) {
	armor += temp;
}
void BaseMonster::changeMoney(int temp) {
	money += temp;
}

//初始化精灵的函数？？？
BaseMonster * BaseMonster::create(const char*  picFileName) {

	BaseMonster * sprite = new BaseMonster();
	// 给英雄初始化血量？金钱？
	if (sprite && initBaseSprite(picFileName))
	{
...
void BaseMonster::Home()//回城函数
{
	...
	auto realMoveTo = MoveTo::create(0.1, Point(0, 0));
