...
*/
BaseTower* BaseTower::createTower(Point point, int color)
{
	auto tower = new BaseTower();
	if (tower&&tower->init(color))
	{
...
bool BaseTower::init(int color)//创建塔对象 要先set颜色才能init
{
	this->setScope(TileCoord*2);//两个瓦片长度
	this->setTowerColor(color);
...
void BaseTower::removeBullet()//攻击！  保证打中
{
	...
	auto instance = GameManager::getInstance();
    //spriteBullet = Sprite::create("");
	//addChild(spriteBullet);//add到哪了！！！！！！！！！！！！！！！！！！！！！！！！！！！！！！坑？？
	...
	//}
	//不管击不击中都消失
	spriteBullet->removeFromParent();//!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!坑？？
	/*if (isMissed)
	{
		//若未击中
	}
